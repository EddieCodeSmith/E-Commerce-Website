...
def update_info(request):
    if request.user.is_authenticated:
        # GETTING CURRENT USER          ur
        current_user = Profile.objects.get(user__id=request.user.id)
        # GETTING CURRENT USER'S SHIPPING INFO
        shipping_user = ShippingAddress.objects.get(user__id=request.user.id)
        # GETTING ORIGINAL USER FORM
        form = UserInfoForm(request.POST or None, instance=current_user)
        # GETTING USER'S SHIPPING FORM
...
def login_user(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            # PERFORMING SO ACTIONS ON THE CART
            current_user = Profile.objects.get(user__id=request.user.id)
            # GETTING SAVED CART FROM DATABASE
            saved_cart = current_user.old_cart
            # CONVERTING DATABASE STRING BACK INTO A DICTIONARY
            if saved_cart:
                ...
                # GETTING THE CART
                cart = Cart(request)
                # LOOPING THROUGH THE CART AND ADDING ITEMS FROM THE DATABASE
...
def logout_user(request):
    logout(request)
    messages.success(request, ("You have successfully logged out... Thanks for stopping by!!!"))
    return redirect('home')
